...
$pyjs['loaded_modules']['main'] = function (__mod_name__) {
	...
	$m['USAGE'] = 'gcbench [num_repetitions] [--depths=N,N,N..] [--threads=N]';
	$m['ENABLE_THREADS'] = true;
	$m['Node'] = (function(){
		var $cls_definition = new Object();
		var $method;
		$cls_definition['__module__'] = 'main';
		$method = $pyjs__bind_method2('__init__', function(l, r) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			if (typeof r == 'undefined') r=arguments['callee']['__args__'][4][1];

			self['left'] = l;
			self['right'] = r;
			return null;
		}
	, 1, [null,null,['self'],['l', null],['r', null]]);
		$cls_definition['__init__'] = $method;
		var $bases = new Array($p['object']);
		var $data = $p['dict']();
		for (var $item in $cls_definition) { $data['__setitem__']($item, $cls_definition[$item]); }
	...
	$m['kMinTreeDepth'] = 4;
	$m['kMaxTreeDepth'] = 16;
	$m['tree_size'] = function(i) {
		var $add2,$sub1,$add1,$sub2;
		return $p['__op_sub']($sub1=(1)<<($p['__op_add']($add1=i,$add2=1)),$sub2=1);
	};
	$m['tree_size']['__name__'] = 'tree_size';

	$m['tree_size']['__bind_type__'] = 0;
	$m['tree_size']['__args__'] = [null,null,['i']];
	$m['num_iters'] = function(i) {
		var $div2,$div1,$mul2,$mul1;
		return (typeof ($div1=(typeof ($mul1=2)==typeof ($mul2=$m['tree_size']($m['kStretchTreeDepth'])) && typeof $mul1=='number'?
			$mul1*$mul2:
			$p['op_mul']($mul1,$mul2)))==typeof ($div2=$m['tree_size'](i)) && typeof $div1=='number' && $div2 !== 0?
			$div1/$div2:
			$p['op_div']($div1,$div2));
		return null;
	};
	$m['num_iters']['__name__'] = 'num_iters';

	$m['num_iters']['__bind_type__'] = 0;
	$m['num_iters']['__args__'] = [null,null,['i']];
	$m['populate'] = function(depth, node) {
		var $sub3,$sub4;
		if ($p['bool'](($p['cmp'](depth, 0) < 1))) {
			return null;
		}
		else {
			depth = $p['__op_sub']($sub3=depth,$sub4=1);
			node['left'] = $m['Node']();
			node['right'] = $m['Node']();
			$m['populate'](depth, $p['getattr'](node, 'left'));
			$m['populate'](depth, $p['getattr'](node, 'right'));
		}
		return null;
	};
	$m['populate']['__name__'] = 'populate';

	$m['populate']['__bind_type__'] = 0;
	$m['populate']['__args__'] = [null,null,['depth'],['node']];
	$m['make_tree'] = function(depth) {
		var $sub8,$sub7,$sub6,$sub5;
		if ($p['bool'](($p['cmp'](depth, 0) < 1))) {
			return $m['Node']();
		}
		else {
			return $m['Node']($m['make_tree']($p['__op_sub']($sub5=depth,$sub6=1)), $m['make_tree']($p['__op_sub']($sub7=depth,$sub8=1)));
		}
		return null;
	};
	$m['make_tree']['__name__'] = 'make_tree';

	$m['make_tree']['__bind_type__'] = 0;
	$m['make_tree']['__args__'] = [null,null,['depth']];
	$m['print_diagnostics'] = function() {

 		return null;
	};
	$m['print_diagnostics']['__name__'] = 'print_diagnostics';

	$m['print_diagnostics']['__bind_type__'] = 0;
	$m['print_diagnostics']['__args__'] = [null,null];
	$m['time_construction'] = function(depth, debug) {
		if (typeof debug == 'undefined') debug=arguments['callee']['__args__'][3][1];
		var $iter2_nextval,$iter1_nextval,$iter1_type,$iter2_iter,i,$iter2_idx,t_start,$iter1_iter,$iter2_type,temp_tree,$iter1_array,niters,t_finish,$iter2_array,$iter1_idx;
		niters = $m['num_iters'](depth);
		$p['printFunc']([$p['sprintf']('Creating %d trees of depth %d', $p['tuple']([niters, depth]))], 1);
		t_start = $m['time']['time']();
		$iter1_iter = $p['range']($p['float_int'](niters));
		$iter1_nextval=$p['__iter_prepare']($iter1_iter,false);
		while (typeof($p['__wrapped_next']($iter1_nextval)['$nextval']) != 'undefined') {
			i = $iter1_nextval['$nextval'];
			temp_tree = $m['Node']();
			$m['populate'](depth, temp_tree);
			temp_tree = null;
		}
		t_finish = $m['time']['time']();
		if ($p['bool'](debug)) {
		}
		t_start = $m['time']['time']();
		$iter2_iter = $p['range']($p['float_int'](niters));
		$iter2_nextval=$p['__iter_prepare']($iter2_iter,false);
		while (typeof($p['__wrapped_next']($iter2_nextval)['$nextval']) != 'undefined') {
			i = $iter2_nextval['$nextval'];
			temp_tree = $m['make_tree'](depth);
			temp_tree = null;
		}
		t_finish = $m['time']['time']();
		if ($p['bool'](debug)) {
	...
	$m['time_construction']['__name__'] = 'time_construction';

	$m['time_construction']['__bind_type__'] = 0;
	$m['time_construction']['__args__'] = [null,null,['depth'],['debug', false]];
	$m['DEFAULT_DEPTHS'] = $p['list']($p['range']($m['kMinTreeDepth'], $p['__op_add']($add3=$m['kMaxTreeDepth'],$add4=1), 2));
	$m['time_constructions'] = function(depths, debug) {
		if (typeof debug == 'undefined') debug=arguments['callee']['__args__'][3][1];
		var $iter3_idx,d,$iter3_nextval,$iter3_type,$iter3_iter,$iter3_array;
		$iter3_iter = depths;
		$iter3_nextval=$p['__iter_prepare']($iter3_iter,false);
		while (typeof($p['__wrapped_next']($iter3_nextval)['$nextval']) != 'undefined') {
			d = $iter3_nextval['$nextval'];
			$m['time_construction'](d, debug);
		}
		return null;
	};
	$m['time_constructions']['__name__'] = 'time_constructions';

	$m['time_constructions']['__bind_type__'] = 0;
	$m['time_constructions']['__args__'] = [null,null,['depths'],['debug', false]];
	$m['time_parallel_constructions'] = function(depths, nthreads, debug) {
		if (typeof debug == 'undefined') debug=arguments['callee']['__args__'][4][1];

		throw ('no thrads!');
		return null;
	};
	$m['time_parallel_constructions']['__name__'] = 'time_parallel_constructions';

	$m['time_parallel_constructions']['__bind_type__'] = 0;
	$m['time_parallel_constructions']['__args__'] = [null,null,['depths'],['nthreads'],['debug', false]];
	$m['main'] = function(numruns, depths, threads, debug) {
		if (typeof depths == 'undefined') depths=arguments['callee']['__args__'][3][1];
		if (typeof threads == 'undefined') threads=arguments['callee']['__args__'][4][1];
		if (typeof debug == 'undefined') debug=arguments['callee']['__args__'][5][1];
		var temp_tree,$iter4_type,array,$iter4_iter,t_finish,$mul3,$sub9,$sub10,$or1,$or2,long_lived_tree,t_start,$iter4_array,$div8,$div3,$mul4,$div6,$div7,$div4,$div5,i,$iter4_nextval,times,$iter4_idx,$add6,$add5;
		times = $p['list']([]);
		$iter4_iter = $p['range'](numruns);
		$iter4_nextval=$p['__iter_prepare']($iter4_iter,false);
		while (typeof($p['__wrapped_next']($iter4_nextval)['$nextval']) != 'undefined') {
			...
			t_start = $m['time']['time']();
			temp_tree = $m['make_tree']($m['kStretchTreeDepth']);
			temp_tree = null;
			$p['printFunc']([$p['sprintf'](' Creating a long-lived binary tree of depth %d', $m['kLongLivedTreeDepth'])], 1);
			long_lived_tree = $m['Node']();
			$m['populate']($m['kLongLivedTreeDepth'], long_lived_tree);
			$p['printFunc']([$p['sprintf'](' Creating a long-lived array of %d doubles', $m['kArraySize'])], 1);
			array = (typeof ($mul3=$p['list']([0.0]))==typeof ($mul4=$m['kArraySize']) && typeof $mul3=='number'?
				$mul3*$mul4:
				$p['op_mul']($mul3,$mul4));
			i = 1;
			while ($p['bool'](($p['cmp'](i, (typeof ($div3=$m['kArraySize'])==typeof ($div4=2) && typeof $div3=='number' && $div4 !== 0?
				$div3/$div4:
				$p['op_div']($div3,$div4))) == -1))) {
				array['__setitem__'](i, (typeof ($div5=1.0)==typeof ($div6=i) && typeof $div5=='number' && $div6 !== 0?
					$div5/$div6:
					$p['op_div']($div5,$div6)));
				i = $p['__op_add']($add5=i,$add6=1);
			}
			if ($p['bool'](threads)) {
				$m['time_parallel_constructions'](depths, threads, debug);
			}
			else {
				$m['time_constructions'](depths, debug);
			}
			if ($p['bool'](($p['bool']($or1=(long_lived_tree === null))?$or1:!$p['op_eq'](array['__getitem__'](1024), (typeof ($div7=1.0)==typeof ($div8=1024) && typeof $div7=='number' && $div8 !== 0?
				$div7/$div8:
				$p['op_div']($div7,$div8)))))) {
				throw ((typeof Failed == "undefined"?$m['Failed']:Failed));
			}
			t_finish = $m['time']['time']();
			$m['print_diagnostics']();
			if ($p['bool'](debug)) {
			}
			times['append']($p['__op_sub']($sub9=t_finish,$sub10=t_start));
		}
		return times;
	};
	...
	$m['main']['__args__'] = [null,null,['numruns'],['depths', $m['DEFAULT_DEPTHS']],['threads', 0],['debug', false]];
	$m['Failed'] = (function(){
		var $cls_definition = new Object();
		var $method;
		$cls_definition['__module__'] = 'main';
		...
		var $data = $p['dict']();
		for (var $item in $cls_definition) { $data['__setitem__']($item, $cls_definition[$item]); }
		return $p['_create_class']('Failed', $p['tuple']($bases), $data);
	})();
	$m['main'](1);
	return this;
}; /* end main */
