...
function gcbench () {
    ...
    */
	...
	(function () {
		...
		var USAGE = 'gcbench [num_repetitions] [--depths=N,N,N..] [--threads=N]';
		var ENABLE_THREADS = true;
		var Node = __class__ ('Node', [object], {
			get __init__ () {return __get__ (this, function (self, l, r) {
				if (typeof l == 'undefined' || (l != null && l .hasOwnProperty ("__kwargtrans__"))) {;
					var l = null;
				};
		...
		var kStretchTreeDepth = 18;
		var kLongLivedTreeDepth = 16;
		var kArraySize = 500000;
		var kMinTreeDepth = 4;
		var kMaxTreeDepth = 16;
		var tree_size = function (i) {
			return __sub__ (__lshift__ (1, __add__ (i, 1)), 1);
		};
		var num_iters = function (i) {
			return __div__ (__mul__ (2, __call__ (tree_size, null, kStretchTreeDepth)), __call__ (tree_size, null, i));
		};
		var populate = function (depth, node) {
			if (__le__ (depth, 0)) {
				return ;
			}
			else {
				var depth = __call__ (__isub__, null, depth, 1);
				node.left = __call__ (Node, null);
				node.right = __call__ (Node, null);
				__call__ (populate, null, depth, node.left);
				__call__ (populate, null, depth, node.right);
			}
		};
		var make_tree = function (depth) {
			if (__le__ (depth, 0)) {
				return __call__ (Node, null);
			}
			else {
				return __call__ (Node, null, __call__ (make_tree, null, __sub__ (depth, 1)), __call__ (make_tree, null, __sub__ (depth, 1)));
			}
		};
		var print_diagnostics = function () {
			// pass;
		};
		var time_construction = function (depth, debug) {
			...
			for (var i = 0; i < __call__ (int, null, niters); i++) {
				var temp_tree = __call__ (Node, null);
				__call__ (populate, null, depth, temp_tree);
				var temp_tree = null;
			}
			...
			var t_start = __call__ (time.time, time);
			for (var i = 0; i < __call__ (int, null, niters); i++) {
				var temp_tree = __call__ (make_tree, null, depth);
				var temp_tree = null;
			}
			var t_finish = __call__ (time.time, time);
			if (debug) {
		...
		var time_constructions = function (depths, debug) {
			if (typeof debug == 'undefined' || (debug != null && debug .hasOwnProperty ("__kwargtrans__"))) {;
				var debug = false;
			};
			var __iterable0__ = depths;
			for (var __index0__ = 0; __index0__ < __iterable0__.length; __index0__++) {
				var d = __getitem__ (__iterable0__, __index0__);
				__call__ (time_construction, null, d, debug);
			}
		};
		var time_parallel_constructions = function (depths, nthreads, debug) {
			if (typeof debug == 'undefined' || (debug != null && debug .hasOwnProperty ("__kwargtrans__"))) {;
				var debug = false;
			};
			var __except0__ = 'no thrads!';
			__except0__.__cause__ = null;
			throw __except0__;
		};
		var main = function (numruns, depths, threads, debug) {
			if (typeof depths == 'undefined' || (depths != null && depths .hasOwnProperty ("__kwargtrans__"))) {;
				var depths = DEFAULT_DEPTHS;
			};
			if (typeof threads == 'undefined' || (threads != null && threads .hasOwnProperty ("__kwargtrans__"))) {;
				var threads = 0;
			};
			if (typeof debug == 'undefined' || (debug != null && debug .hasOwnProperty ("__kwargtrans__"))) {;
				var debug = false;
			...
			for (var i = 0; i < numruns; i++) {
				...
				var i = 1;
				while (__lt__ (i, __div__ (kArraySize, 2))) {
				...
				if (threads) {
					...
					__call__ (time_constructions, null, depths, debug);
				}
				if (long_lived_tree === null || __ne__ (__getitem__ (array, 1024), __div__ (1.0, 1024))) {
					var __except0__ = Failed;
					__except0__.__cause__ = null;
					throw __except0__;
				}
				var t_finish = __call__ (time.time, time);
		...
		var Failed = __class__ ('Failed', [Exception], {
		});
		__call__ (main, null, 1);
		__pragma__ ('<use>' +
			'time' +
		'</use>')
		__pragma__ ('<all>')
			__all__.DEFAULT_DEPTHS = DEFAULT_DEPTHS;
			__all__.ENABLE_THREADS = ENABLE_THREADS;
			__all__.Failed = Failed;
			__all__.Node = Node;
			__all__.USAGE = USAGE;
			__all__.kArraySize = kArraySize;
			__all__.kLongLivedTreeDepth = kLongLivedTreeDepth;
			__all__.kMaxTreeDepth = kMaxTreeDepth;
			__all__.kMinTreeDepth = kMinTreeDepth;
			__all__.kStretchTreeDepth = kStretchTreeDepth;
			__all__.main = main;
			__all__.make_tree = make_tree;
			__all__.num_iters = num_iters;
			__all__.populate = populate;
			__all__.print_diagnostics = print_diagnostics;
			__all__.time_construction = time_construction;
			__all__.time_constructions = time_constructions;
			__all__.time_parallel_constructions = time_parallel_constructions;
			__all__.tree_size = tree_size;
		__pragma__ ('</all>')
	}) ();
   return __all__;
}
