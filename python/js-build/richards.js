"use strict";
// Transcrypt'ed from Python, 2017-07-27 19:25:29
function richards () {
    ...
    */
	...
	(function () {
		...
		var TaskRec = __class__ ('TaskRec', [object], {
		});
		var DeviceTaskRec = __class__ ('DeviceTaskRec', [TaskRec], {
			get __init__ () {return __get__ (this, function (self) {
				self.pending = null;
			});}
		});
		var IdleTaskRec = __class__ ('IdleTaskRec', [TaskRec], {
			get __init__ () {return __get__ (this, function (self) {
				self.control = 1;
				self.count = 10000;
			});}
		});
		var HandlerTaskRec = __class__ ('HandlerTaskRec', [TaskRec], {
			get __init__ () {return __get__ (this, function (self) {
				self.work_in = null;
				self.device_in = null;
			});},
		...
		var WorkerTaskRec = __class__ ('WorkerTaskRec', [TaskRec], {
			get __init__ () {return __get__ (this, function (self) {
				self.destination = I_HANDLERA;
				self.count = 0;
			});}
		});
		var TaskState = __class__ ('TaskState', [object], {
			get __init__ () {return __get__ (this, function (self) {
				self.packet_pending = true;
				self.task_waiting = false;
				self.task_holding = false;
			});},
			get packetPending () {return __get__ (this, function (self) {
				self.packet_pending = true;
				self.task_waiting = false;
				self.task_holding = false;
				return self;
			});},
			get waiting () {return __get__ (this, function (self) {
				self.packet_pending = false;
				self.task_waiting = true;
				self.task_holding = false;
				return self;
			});},
			get running () {return __get__ (this, function (self) {
				self.packet_pending = false;
				self.task_waiting = false;
				self.task_holding = false;
				return self;
			});},
			get waitingWithPacket () {return __get__ (this, function (self) {
				self.packet_pending = true;
				self.task_waiting = true;
				self.task_holding = false;
				return self;
			});},
			get isPacketPending () {return __get__ (this, function (self) {
				return self.packet_pending;
			});},
			get isTaskWaiting () {return __get__ (this, function (self) {
				return self.task_waiting;
			});},
			get isTaskHolding () {return __get__ (this, function (self) {
				return self.task_holding;
			});},
			get isTaskHoldingOrWaiting () {return __get__ (this, function (self) {
				return self.task_holding || !(self.packet_pending) && self.task_waiting;
			});},
			get isWaitingWithPacket () {return __get__ (this, function (self) {
				return self.packet_pending && self.task_waiting && !(self.task_holding);
			});}
		});
		var tracing = false;
		var layout = 0;
		var trace = function (a) {
			...
			__call__ (print, null, a, __kwargtrans__ ({end: ' '}));
		};
		var TASKTABSIZE = 10;
		...
		var Task = __class__ ('Task', [TaskState], {
			...
			get addPacket () {return __get__ (this, function (self, p, old) {
				if (self.input === null) {
					self.input = p;
					self.packet_pending = true;
		...
		var schedule = function () {
			...
			while (t !== null) {
				var pkt = null;
				if (tracing) {
					__call__ (print, null, 'tcb =', t.ident);
				}
		...
		var Richards = __class__ ('Richards', [object], {
			get run () {return __get__ (this, function (self, iterations) {
		...
		var main = function (entry_point, iterations) {
			if (typeof entry_point == 'undefined' || (entry_point != null && entry_point .hasOwnProperty ("__kwargtrans__"))) {;
				var entry_point = entry_point;
			};
			if (typeof iterations == 'undefined' || (iterations != null && iterations .hasOwnProperty ("__kwargtrans__"))) {;
				var iterations = 10;
			};
			__call__ (print, null, 'Richards benchmark (Python) starting... [%r]', entry_point);
			var __left0__ = __call__ (entry_point, null, iterations);
			var result = __left0__ [0];
			var startTime = __left0__ [1];
			var endTime = __left0__ [2];
			if (!(result)) {
				__call__ (print, null, 'Incorrect results!');
				return __neg__ (1);
			}
			__call__ (print, null, 'finished.');
			var total_s = __sub__ (endTime, startTime);
			__call__ (print, null, 'Total time for %d iterations: %.2f secs', iterations, total_s);
			__call__ (print, null, 'Average time per iteration: %.2f ms', __div__ (__mul__ (total_s, 1000), iterations));
			return 42;
		};
		...
		__pragma__ ('<all>')
			__all__.A = A;
			__all__.BUFSIZE = BUFSIZE;
			__all__.BUFSIZE_RANGE = BUFSIZE_RANGE;
			__all__.DeviceTask = DeviceTask;
			__all__.DeviceTaskRec = DeviceTaskRec;
			__all__.HandlerTask = HandlerTask;
			__all__.HandlerTaskRec = HandlerTaskRec;
			__all__.I_DEVA = I_DEVA;
			__all__.I_DEVB = I_DEVB;
			__all__.I_HANDLERA = I_HANDLERA;
			__all__.I_HANDLERB = I_HANDLERB;
			__all__.I_IDLE = I_IDLE;
			__all__.I_WORK = I_WORK;
			__all__.IdleTask = IdleTask;
			__all__.IdleTaskRec = IdleTaskRec;
			__all__.K_DEV = K_DEV;
			__all__.K_WORK = K_WORK;
			__all__.Packet = Packet;
			__all__.Richards = Richards;
			__all__.TASKTABSIZE = TASKTABSIZE;
			__all__.Task = Task;
			__all__.TaskRec = TaskRec;
			__all__.TaskState = TaskState;
			__all__.TaskWorkArea = TaskWorkArea;
			__all__.WorkTask = WorkTask;
			__all__.WorkerTaskRec = WorkerTaskRec;
			__all__.entry_point = entry_point;
			__all__.layout = layout;
			__all__.main = main;
			__all__.schedule = schedule;
			__all__.taskWorkArea = taskWorkArea;
			__all__.trace = trace;
			__all__.tracing = tracing;
		__pragma__ ('</all>')
	}) ();
   return __all__;
}
