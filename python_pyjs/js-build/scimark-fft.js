...
$pyjs['loaded_modules']['main'] = function (__mod_name__) {
	...
	$m['math'] = $p['___import___']('math', null);
	$m['array'] = function(type, init) {
		if (typeof init == 'undefined') init=arguments['callee']['__args__'][3][1];
	...
	$m['array']['__args__'] = [null,null,['type'],['init', $p['list']([])]];
	$m['Array2D'] = (function(){
		var $cls_definition = new Object();
		var $method;
		$cls_definition['__module__'] = 'main';
		$method = $pyjs__bind_method2('__init__', function(w, h, data) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			if (typeof data == 'undefined') data=arguments['callee']['__args__'][5][1];
			var $mul3,$mul4,$mul2,$mul1;
			self['width'] = w;
			self['height'] = h;
			self['data'] = (typeof ($mul3=$m['array']('d', $p['list']([0])))==typeof ($mul4=(typeof ($mul1=w)==typeof ($mul2=h) && typeof $mul1=='number'?
				$mul1*$mul2:
	...
	, 1, [null,null,['self'],['w'],['h'],['data', null]]);
		$cls_definition['__init__'] = $method;
		$method = $pyjs__bind_method2('_idx', function(x, y) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			var $and1,$add2,$add1,$mul6,$mul5,$and2;
			if ($p['bool'](($p['bool']($and1=(($p['cmp'](0, ($compare1 = x)) < 1)&&($p['cmp']($compare1, ($compare2 = $p['getattr'](self, 'width'))) == -1)))?(($p['cmp'](0, ($compare3 = y)) < 1)&&($p['cmp']($compare3, ($compare4 = $p['getattr'](self, 'height'))) == -1)):$and1))) {
				return $p['__op_add']($add1=(typeof ($mul5=y)==typeof ($mul6=$p['getattr'](self, 'width')) && typeof $mul5=='number'?
					$mul5*$mul6:
					$p['op_mul']($mul5,$mul6)),$add2=x);
			}
			throw ($p['IndexError']);
			return null;
		}
	, 1, [null,null,['self'],['x'],['y']]);
		$cls_definition['_idx'] = $method;
		$method = $pyjs__bind_method2('__getitem__', function(x, y) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			return $p['getattr'](self, 'data')['__getitem__'](self['_idx'](x, y));
		}
	, 1, [null,null,['self'],['x'],['y']]);
		$cls_definition['__getitem__'] = $method;
		$method = $pyjs__bind_method2('__setitem__', function(x, y, val) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			$p['getattr'](self, 'data')['__setitem__'](self['_idx'](x, y), val);
			return null;
		}
	, 1, [null,null,['self'],['x'],['y'],['val']]);
		$cls_definition['__setitem__'] = $method;
		$method = $pyjs__bind_method2('__cmp__', function(other) {
			if (this['__is_instance__'] === true) {
				var self = this;
			...
			return $p['cmp']($p['getattr'](self, 'data'), $p['getattr'](other, 'data'));
		}
	, 1, [null,null,['self'],['other']]);
		$cls_definition['__cmp__'] = $method;
		$method = $pyjs__bind_method2('setup', function(data) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			var $iter2_nextval,$iter1_nextval,$iter1_type,$iter2_iter,$iter1_iter,$iter2_idx,$iter1_array,x,y,$iter2_type,$iter2_array,$iter1_idx;
			$iter1_iter = $p['xrange']($p['getattr'](self, 'height'));
			$iter1_nextval=$p['__iter_prepare']($iter1_iter,false);
			while (typeof($p['__wrapped_next']($iter1_nextval)['$nextval']) != 'undefined') {
				y = $iter1_nextval['$nextval'];
				$iter2_iter = $p['xrange']($p['getattr'](self, 'width'));
				$iter2_nextval=$p['__iter_prepare']($iter2_iter,false);
				while (typeof($p['__wrapped_next']($iter2_nextval)['$nextval']) != 'undefined') {
					x = $iter2_nextval['$nextval'];
					self['__setitem__']($p['tuple']([x, y]), data['__getitem__'](y)['__getitem__'](x));
				}
			}
			return self;
		}
	, 1, [null,null,['self'],['data']]);
		...
		$method = $pyjs__bind_method2('indexes', function() {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			}
			var $iter5_nextval,$iter6_type,$iter5_idx,$iter3_array,$iter5_iter,$iter4_type,$iter5_type,$iter6_iter,$iter4_iter,$iter6_nextval,$iter3_idx,$iter3_iter,$iter5_array,$iter6_idx,y,$iter6_array,$iter3_type,$iter4_nextval,$iter4_idx,$iter4_array,x,$iter3_nextval;
var $generator_state = [0], $generator_exc = [null], $yield_value = null, $exc = null, $is_executing=false;
			var $generator = function () {};
			$generator['next'] = function (noStop) {
			...
			$generator['$genfunc'] = function () {
				...
				if (typeof $generator_state[0] == 'undefined' || $generator_state[0] === 0) {
					...
					$iter5_iter = $p['xrange']($p['getattr'](self, 'height'));
					$iter5_nextval=$p['__iter_prepare']($iter5_iter,false);
					$generator_state[0]=1;
				}
				if ($generator_state[0] == 1) {
				...
				if ($generator_state[0] == 2) {
					for (;($generator_state[1] > 0 || typeof($p['__wrapped_next']($iter5_nextval)['$nextval']) != 'undefined');$generator_state[1] = 0) {
						if (typeof $generator_state[1] == 'undefined' || $generator_state[1] === 0) {
							...
							$iter6_iter = $p['xrange']($p['getattr'](self, 'width'));
							$iter6_nextval=$p['__iter_prepare']($iter6_iter,false);
							$generator_state[1]=1;
						}
						if ($generator_state[1] == 1) {
	...
	, 1, [null,null,['self']]);
		$cls_definition['indexes'] = $method;
		$method = $pyjs__bind_method2('copy_data_from', function(other) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			$p['__setslice']($p['getattr'](self, 'data'), 0, null, $p['__getslice']($p['getattr'](other, 'data'), 0, null));
			return null;
		}
	, 1, [null,null,['self'],['other']]);
		$cls_definition['copy_data_from'] = $method;
		var $bases = new Array($p['object']);
		var $data = $p['dict']();
		for (var $item in $cls_definition) { $data['__setitem__']($item, $cls_definition[$item]); }
		return $p['_create_class']('Array2D', $p['tuple']($bases), $data);
	})();
	$m['Random'] = (function(){
		var $cls_definition = new Object();
		var $method;
		$cls_definition['__module__'] = 'main';
		$cls_definition['MDIG'] = 32;
		$cls_definition['ONE'] = 1;
		$cls_definition['m1'] = $p['__op_add']($add3=($cls_definition['ONE'])<<($p['__op_sub']($sub1=$cls_definition['MDIG'],$sub2=2)),$add4=$p['__op_sub']($sub5=($cls_definition['ONE'])<<($p['__op_sub']($sub3=$cls_definition['MDIG'],$sub4=2)),$sub6=$cls_definition['ONE']));
		$cls_definition['m2'] = ($cls_definition['ONE'])<<((typeof ($div1=$cls_definition['MDIG'])==typeof ($div2=2) && typeof $div1=='number' && $div2 !== 0?
			$div1/$div2:
			$p['op_div']($div1,$div2)));
		$cls_definition['dm1'] = (typeof ($div3=1.0)==typeof ($div4=$p['float']($cls_definition['m1'])) && typeof $div3=='number' && $div4 !== 0?
			$div3/$div4:
			$p['op_div']($div3,$div4));
		$method = $pyjs__bind_method2('__init__', function(seed) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
	...
	, 1, [null,null,['self'],['seed']]);
		$cls_definition['__init__'] = $method;
		$method = $pyjs__bind_method2('initialize', function(seed) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			var $iter7_idx,j0,j1,$mod5,$mod4,$mod7,$mod6,$mod1,$mod3,$mod2,$mod9,$sub8,$mul14,$div10,$div11,$div12,$mod8,k1,k0,$iter7_type,$sub7,$mod10,iloop,jseed,$iter7_iter,$div8,$div9,$add10,$mul9,$mul8,$mul7,$div6,$div7,$div5,$mul16,$mul15,$mul12,$mul13,$mul11,$iter7_nextval,$mul10,$iter7_array,$add6,$add7,$add5,$add8,$add9;
			self['seed'] = seed;
			seed = $p['abs'](seed);
			jseed = $p['min'](seed, $p['getattr'](self, 'm1'));
			if ($p['bool']($p['op_eq']((typeof ($mod1=jseed)==typeof ($mod2=2) && typeof $mod1=='number'?
				(($mod1=$mod1%$mod2)<0&&$mod2>0?$mod1+$mod2:$mod1):
				$p['op_mod']($mod1,$mod2)), 0))) {
				jseed = $p['__op_sub']($sub7=jseed,$sub8=1);
			}
			k0 = (typeof ($mod3=9069)==typeof ($mod4=$p['getattr'](self, 'm2')) && typeof $mod3=='number'?
				(($mod3=$mod3%$mod4)<0&&$mod4>0?$mod3+$mod4:$mod3):
				$p['op_mod']($mod3,$mod4));
			k1 = (typeof ($div5=9069)==typeof ($div6=$p['getattr'](self, 'm2')) && typeof $div5=='number' && $div6 !== 0?
				$div5/$div6:
				$p['op_div']($div5,$div6));
			j0 = (typeof ($mod5=jseed)==typeof ($mod6=$p['getattr'](self, 'm2')) && typeof $mod5=='number'?
				(($mod5=$mod5%$mod6)<0&&$mod6>0?$mod5+$mod6:$mod5):
				$p['op_mod']($mod5,$mod6));
			j1 = (typeof ($div7=jseed)==typeof ($div8=$p['getattr'](self, 'm2')) && typeof $div7=='number' && $div8 !== 0?
				$div7/$div8:
				$p['op_div']($div7,$div8));
			self['m'] = (typeof ($mul7=$m['array']('d', $p['list']([0])))==typeof ($mul8=17) && typeof $mul7=='number'?
				$mul7*$mul8:
				$p['op_mul']($mul7,$mul8));
			$iter7_iter = $p['xrange'](17);
			$iter7_nextval=$p['__iter_prepare']($iter7_iter,false);
			while (typeof($p['__wrapped_next']($iter7_nextval)['$nextval']) != 'undefined') {
				iloop = $iter7_nextval['$nextval'];
				jseed = (typeof ($mul9=j0)==typeof ($mul10=k0) && typeof $mul9=='number'?
					$mul9*$mul10:
					$p['op_mul']($mul9,$mul10));
				j1 = (typeof ($mod7=$p['__op_add']($add7=$p['__op_add']($add5=(typeof ($div9=jseed)==typeof ($div10=$p['getattr'](self, 'm2')) && typeof $div9=='number' && $div10 !== 0?
					$div9/$div10:
					$p['op_div']($div9,$div10)),$add6=(typeof ($mul11=j0)==typeof ($mul12=k1) && typeof $mul11=='number'?
					$mul11*$mul12:
					$p['op_mul']($mul11,$mul12))),$add8=(typeof ($mul13=j1)==typeof ($mul14=k0) && typeof $mul13=='number'?
					$mul13*$mul14:
					$p['op_mul']($mul13,$mul14))))==typeof ($mod8=(typeof ($div11=$p['getattr'](self, 'm2'))==typeof ($div12=2) && typeof $div11=='number' && $div12 !== 0?
					$div11/$div12:
					$p['op_div']($div11,$div12))) && typeof $mod7=='number'?
					(($mod7=$mod7%$mod8)<0&&$mod8>0?$mod7+$mod8:$mod7):
					$p['op_mod']($mod7,$mod8));
				j0 = (typeof ($mod9=jseed)==typeof ($mod10=$p['getattr'](self, 'm2')) && typeof $mod9=='number'?
					(($mod9=$mod9%$mod10)<0&&$mod10>0?$mod9+$mod10:$mod9):
					$p['op_mod']($mod9,$mod10));
				$p['getattr'](self, 'm')['__setitem__'](iloop, $p['__op_add']($add9=j0,$add10=(typeof ($mul15=$p['getattr'](self, 'm2'))==typeof ($mul16=j1) && typeof $mul15=='number'?
					$mul15*$mul16:
	...
	, 1, [null,null,['self'],['seed']]);
		...
		$method = $pyjs__bind_method2('nextDouble', function() {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			}
			var $mul17,$sub10,I,$sub12,J,m,$mul19,$sub14,$add14,$sub13,$sub9,$add11,$add12,$add13,$sub11,$mul18,$mul22,k,$mul20,$mul21;
			var $tupleassign1 = $p['__ass_unpack']($p['tuple']([$p['getattr'](self, 'i'), $p['getattr'](self, 'j'), $p['getattr'](self, 'm')]), 3, null);
			I = $tupleassign1[0];
			J = $tupleassign1[1];
			m = $tupleassign1[2];
			k = $p['__op_sub']($sub9=m['__getitem__'](I),$sub10=m['__getitem__'](J));
			if ($p['bool'](($p['cmp'](k, 0) == -1))) {
				k = $p['__op_add']($add11=k,$add12=$p['getattr'](self, 'm1'));
			}
			$p['getattr'](self, 'm')['__setitem__'](J, k);
			...
			self['i'] = I;
			if ($p['bool']($p['op_eq'](J, 0))) {
				J = 16;
			}
			else {
				J = $p['__op_sub']($sub13=J,$sub14=1);
			}
			self['j'] = J;
			if ($p['bool']($p['getattr'](self, 'haveRange'))) {
				return $p['__op_add']($add13=$p['getattr'](self, 'left'),$add14=(typeof ($mul19=(typeof ($mul17=$p['getattr'](self, 'dm1'))==typeof ($mul18=$p['float'](k)) && typeof $mul17=='number'?
					$mul17*$mul18:
					$p['op_mul']($mul17,$mul18)))==typeof ($mul20=$p['getattr'](self, 'width')) && typeof $mul19=='number'?
					$mul19*$mul20:
					$p['op_mul']($mul19,$mul20)));
			}
			else {
				return (typeof ($mul21=$p['getattr'](self, 'dm1'))==typeof ($mul22=$p['float'](k)) && typeof $mul21=='number'?
					$mul21*$mul22:
	...
	, 1, [null,null,['self']]);
		$cls_definition['nextDouble'] = $method;
		$method = $pyjs__bind_method2('RandomMatrix', function(a) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			var $iter8_idx,$iter8_array,$iter8_iter,$iter8_nextval,y,x,$iter8_type;
			$iter8_iter = a['indexes']();
			$iter8_nextval=$p['__iter_prepare']($iter8_iter,false);
			while (typeof($p['__wrapped_next']($iter8_nextval)['$nextval']) != 'undefined') {
				var $tupleassign2 = $p['__ass_unpack']($iter8_nextval['$nextval'], 2, null);
				x = $tupleassign2[0];
				y = $tupleassign2[1];
				a['__setitem__']($p['tuple']([x, y]), self['nextDouble']());
			}
			return a;
		}
	, 1, [null,null,['self'],['a']]);
		$cls_definition['RandomMatrix'] = $method;
		$method = $pyjs__bind_method2('RandomVector', function(n) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			return $m['array']('d', function(){
				var $iter9_iter,i,$iter9_nextval,$iter9_idx,$collcomp1,$iter9_type,$iter9_array;
	$collcomp1 = $p['list']();
			$iter9_iter = $p['xrange'](n);
			$iter9_nextval=$p['__iter_prepare']($iter9_iter,false);
			while (typeof($p['__wrapped_next']($iter9_nextval)['$nextval']) != 'undefined') {
				i = $iter9_nextval['$nextval'];
				$collcomp1['append'](self['nextDouble']());
			}

	return $collcomp1;}());
		}
	, 1, [null,null,['self'],['n']]);
		$cls_definition['RandomVector'] = $method;
		var $bases = new Array($p['object']);
		var $data = $p['dict']();
		for (var $item in $cls_definition) { $data['__setitem__']($item, $cls_definition[$item]); }
		return $p['_create_class']('Random', $p['tuple']($bases), $data);
	})();
	$m['ArrayList'] = (function(){
		var $cls_definition = new Object();
		var $method;
		$cls_definition['__module__'] = 'main';
		$method = $pyjs__bind_method2('__init__', function(w, h, data) {
			...
			self['height'] = h;
			self['data'] = function(){
				var $iter10_nextval,$iter10_idx,$collcomp2,$iter10_array,y,$mul24,$iter10_type,$iter10_iter,$mul23;
	$collcomp2 = $p['list']();
			$iter10_iter = $p['xrange'](h);
			$iter10_nextval=$p['__iter_prepare']($iter10_iter,false);
			while (typeof($p['__wrapped_next']($iter10_nextval)['$nextval']) != 'undefined') {
				y = $iter10_nextval['$nextval'];
				$collcomp2['append']((typeof ($mul23=$m['array']('d', $p['list']([0])))==typeof ($mul24=w) && typeof $mul23=='number'?
					$mul23*$mul24:
	...
	, 1, [null,null,['self'],['w'],['h'],['data', null]]);
		...
		$method = $pyjs__bind_method2('__getitem__', function(idx) {
			...
			if ($p['bool']($p['isinstance'](idx, $p['tuple']))) {
				return $p['getattr'](self, 'data')['__getitem__'](idx['__getitem__'](1))['__getitem__'](idx['__getitem__'](0));
			}
			else {
				return $p['getattr'](self, 'data')['__getitem__'](idx);
			}
			return null;
		}
	, 1, [null,null,['self'],['idx']]);
		$cls_definition['__getitem__'] = $method;
		$method = $pyjs__bind_method2('__setitem__', function(idx, val) {
			if (this['__is_instance__'] === true) {
				var self = this;
			} else {
				var self = arguments[0];
			...
			if ($p['bool']($p['isinstance'](idx, $p['tuple']))) {
				$p['getattr'](self, 'data')['__getitem__'](idx['__getitem__'](1))['__setitem__'](idx['__getitem__'](0), val);
			}
			else {
				$p['getattr'](self, 'data')['__setitem__'](idx, val);
			}
			return null;
		}
	, 1, [null,null,['self'],['idx'],['val']]);
		$cls_definition['__setitem__'] = $method;
		$method = $pyjs__bind_method2('copy_data_from', function(other) {
			if (this['__is_instance__'] === true) {
				var self = this;
			...
			var $iter11_iter,$iter11_type,l1,l2,$iter11_array,$iter11_nextval,$iter11_idx;
			$iter11_iter = $p['zip']($p['getattr'](self, 'data'), $p['getattr'](other, 'data'));
			$iter11_nextval=$p['__iter_prepare']($iter11_iter,false);
			while (typeof($p['__wrapped_next']($iter11_nextval)['$nextval']) != 'undefined') {
				var $tupleassign3 = $p['__ass_unpack']($iter11_nextval['$nextval'], 2, null);
				l1 = $tupleassign3[0];
				l2 = $tupleassign3[1];
				$p['__setslice'](l1, 0, null, l2);
			}
			return null;
		}
	, 1, [null,null,['self'],['other']]);
		$cls_definition['copy_data_from'] = $method;
		var $bases = new Array($m['Array2D']);
		var $data = $p['dict']();
		for (var $item in $cls_definition) { $data['__setitem__']($item, $cls_definition[$item]); }
		return $p['_create_class']('ArrayList', $p['tuple']($bases), $data);
	})();
	$m['SOR_execute'] = function(omega, G, num_iterations) {
		var $sub22,$sub23,$sub20,$sub21,$sub24,$sub19,$mul30,$iter13_idx,$add21,$add18,$add23,$add22,$add25,$add24,$add26,$mul28,$mul29,$mul26,$mul27,$mul25,$iter13_type,$iter14_array,$sub18,$iter14_type,$sub17,$iter12_array,$sub16,$sub15,$add15,$add16,$add17,$iter14_iter,$add19,$iter14_idx,$iter14_nextval,$iter13_nextval,$iter13_iter,$iter12_type,$iter13_array,$iter12_iter,p,$add20,y,x,$iter12_idx,$iter12_nextval;
		$iter12_iter = $p['xrange'](num_iterations);
		$iter12_nextval=$p['__iter_prepare']($iter12_iter,false);
		while (typeof($p['__wrapped_next']($iter12_nextval)['$nextval']) != 'undefined') {
			p = $iter12_nextval['$nextval'];
			$iter13_iter = $p['xrange'](1, $p['__op_sub']($sub15=$p['getattr'](G, 'height'),$sub16=1));
			$iter13_nextval=$p['__iter_prepare']($iter13_iter,false);
			while (typeof($p['__wrapped_next']($iter13_nextval)['$nextval']) != 'undefined') {
				y = $iter13_nextval['$nextval'];
				$iter14_iter = $p['xrange'](1, $p['__op_sub']($sub17=$p['getattr'](G, 'width'),$sub18=1));
				$iter14_nextval=$p['__iter_prepare']($iter14_iter,false);
				while (typeof($p['__wrapped_next']($iter14_nextval)['$nextval']) != 'undefined') {
					x = $iter14_nextval['$nextval'];
					G['__setitem__']($p['tuple']([x, y]), $p['__op_add']($add25=(typeof ($mul27=(typeof ($mul25=omega)==typeof ($mul26=0.25) && typeof $mul25=='number'?
						$mul25*$mul26:
						$p['op_mul']($mul25,$mul26)))==typeof ($mul28=$p['__op_add']($add23=$p['__op_add']($add19=$p['__op_add']($add17=G['__getitem__']($p['tuple']([x, $p['__op_sub']($sub19=y,$sub20=1)])),$add18=G['__getitem__']($p['tuple']([x, $p['__op_add']($add15=y,$add16=1)]))),$add20=G['__getitem__']($p['tuple']([$p['__op_sub']($sub21=x,$sub22=1), y]))),$add24=G['__getitem__']($p['tuple']([$p['__op_add']($add21=x,$add22=1), y])))) && typeof $mul27=='number'?
						$mul27*$mul28:
						$p['op_mul']($mul27,$mul28)),$add26=(typeof ($mul29=$p['__op_sub']($sub23=1.0,$sub24=omega))==typeof ($mul30=G['__getitem__']($p['tuple']([x, y]))) && typeof $mul29=='number'?
						$mul29*$mul30:
	...
	$m['SOR_execute']['__name__'] = 'SOR_execute';

	$m['SOR_execute']['__bind_type__'] = 0;
	$m['SOR_execute']['__args__'] = [null,null,['omega'],['G'],['num_iterations']];
	$m['SOR'] = function(args) {
		var a,Array,cycles,n;
		var $tupleassign4 = $p['__ass_unpack']($p['map']((typeof eval == "undefined"?$m['eval']:eval), args), 3, null);
		n = $tupleassign4[0];
		cycles = $tupleassign4[1];
		Array = $tupleassign4[2];
		a = Array(n, n);
		$m['SOR_execute'](1.25, a, cycles);
		return $p['sprintf']('SOR(%d, %d)', $p['tuple']([n, cycles]));
	};
	$m['SOR']['__name__'] = 'SOR';

	$m['SOR']['__bind_type__'] = 0;
	$m['SOR']['__args__'] = [null,null,['args']];
	$m['SparseCompRow_matmult'] = function(M, y, val, row, col, x, num_iterations) {
		var $iter16_idx,$iter17_type,$iter16_iter,$add29,$add28,$iter17_iter,$iter15_iter,$iter16_type,$add27,$iter16_nextval,$iter17_nextval,reps,$iter15_array,$iter17_array,$iter17_idx,$iter16_array,i,$add30,$iter15_idx,r,$iter15_nextval,$iter15_type,sa,$mul31,$mul32;
		$iter15_iter = $p['xrange'](num_iterations);
		$iter15_nextval=$p['__iter_prepare']($iter15_iter,false);
		while (typeof($p['__wrapped_next']($iter15_nextval)['$nextval']) != 'undefined') {
			reps = $iter15_nextval['$nextval'];
			$iter16_iter = $p['xrange'](M);
			$iter16_nextval=$p['__iter_prepare']($iter16_iter,false);
			while (typeof($p['__wrapped_next']($iter16_nextval)['$nextval']) != 'undefined') {
				r = $iter16_nextval['$nextval'];
				sa = 0.0;
				$iter17_iter = $p['xrange'](row['__getitem__'](r), row['__getitem__']($p['__op_add']($add27=r,$add28=1)));
				$iter17_nextval=$p['__iter_prepare']($iter17_iter,false);
				while (typeof($p['__wrapped_next']($iter17_nextval)['$nextval']) != 'undefined') {
					i = $iter17_nextval['$nextval'];
					sa = $p['__op_add']($add29=sa,$add30=(typeof ($mul31=x['__getitem__'](col['__getitem__'](i)))==typeof ($mul32=val['__getitem__'](i)) && typeof $mul31=='number'?
						$mul31*$mul32:
	...
	$m['SparseCompRow_matmult']['__name__'] = 'SparseCompRow_matmult';

	$m['SparseCompRow_matmult']['__bind_type__'] = 0;
	$m['SparseCompRow_matmult']['__args__'] = [null,null,['M'],['y'],['val'],['row'],['col'],['x'],['num_iterations']];
	$m['SparseMatMult'] = function(args) {
		var result,$mul43,anz,row,rowr,i,val,$div14,$div15,$div16,$mul40,$mul41,$mul42,$div13,$iter19_iter,nz,$iter19_type,$iter19_nextval,nr,$iter18_nextval,$iter18_type,$iter18_iter,$mul46,N,$mul35,step,$iter19_idx,$mul34,$iter19_array,$iter18_idx,$add38,cycles,$iter18_array,$add32,$add33,$add31,$add36,$add37,$add34,$add35,$mul44,r,$mul39,$mul38,$mul45,y,x,$mul37,$mul36,col,$mul33;
		var $tupleassign5 = $p['__ass_unpack']($p['map']($p['float_int'], args), 3, null);
		N = $tupleassign5[0];
		nz = $tupleassign5[1];
		cycles = $tupleassign5[2];
		x = (typeof ($mul33=$m['array']('d', $p['list']([0])))==typeof ($mul34=N) && typeof $mul33=='number'?
			$mul33*$mul34:
			$p['op_mul']($mul33,$mul34));
		y = (typeof ($mul35=$m['array']('d', $p['list']([0])))==typeof ($mul36=N) && typeof $mul35=='number'?
			$mul35*$mul36:
			$p['op_mul']($mul35,$mul36));
		result = 0.0;
		nr = (typeof ($div13=nz)==typeof ($div14=N) && typeof $div13=='number' && $div14 !== 0?
			$div13/$div14:
			$p['op_div']($div13,$div14));
		anz = (typeof ($mul37=nr)==typeof ($mul38=N) && typeof $mul37=='number'?
			$mul37*$mul38:
			$p['op_mul']($mul37,$mul38));
		val = (typeof ($mul39=$m['array']('d', $p['list']([0])))==typeof ($mul40=anz) && typeof $mul39=='number'?
			$mul39*$mul40:
			$p['op_mul']($mul39,$mul40));
		col = (typeof ($mul41=$m['array']('i', $p['list']([0])))==typeof ($mul42=nz) && typeof $mul41=='number'?
			$mul41*$mul42:
			$p['op_mul']($mul41,$mul42));
		row = (typeof ($mul43=$m['array']('i', $p['list']([0])))==typeof ($mul44=$p['__op_add']($add31=N,$add32=1)) && typeof $mul43=='number'?
			$mul43*$mul44:
			$p['op_mul']($mul43,$mul44));
		row['__setitem__'](0, 0);
		$iter18_iter = $p['xrange'](N);
		$iter18_nextval=$p['__iter_prepare']($iter18_iter,false);
		while (typeof($p['__wrapped_next']($iter18_nextval)['$nextval']) != 'undefined') {
			r = $iter18_nextval['$nextval'];
			rowr = row['__getitem__'](r);
			step = (typeof ($div15=r)==typeof ($div16=nr) && typeof $div15=='number' && $div16 !== 0?
				$div15/$div16:
				$p['op_div']($div15,$div16));
			row['__setitem__']($p['__op_add']($add35=r,$add36=1), $p['__op_add']($add33=rowr,$add34=nr));
			if ($p['bool'](($p['cmp'](step, 1) == -1))) {
				step = 1;
			}
			$iter19_iter = $p['xrange'](nr);
			$iter19_nextval=$p['__iter_prepare']($iter19_iter,false);
			while (typeof($p['__wrapped_next']($iter19_nextval)['$nextval']) != 'undefined') {
				i = $iter19_nextval['$nextval'];
				col['__setitem__']($p['__op_add']($add37=rowr,$add38=i), (typeof ($mul45=i)==typeof ($mul46=step) && typeof $mul45=='number'?
					$mul45*$mul46:
					$p['op_mul']($mul45,$mul46)));
			}
		}
		$m['SparseCompRow_matmult'](N, y, val, row, col, x, cycles);
		return $p['sprintf']('SparseMatMult(%d, %d, %d)', $p['tuple']([N, nz, cycles]));
	};
	$m['SparseMatMult']['__name__'] = 'SparseMatMult';

	$m['SparseMatMult']['__bind_type__'] = 0;
	$m['SparseMatMult']['__args__'] = [null,null,['args']];
	$m['MonteCarlo_integrate'] = function(Num_samples) {
		var $iter20_nextval,$mul47,$iter20_array,$mul48,$mul49,$div17,$add42,$add41,$add40,$iter20_iter,rnd,$add39,$div18,count,$iter20_type,under_curve,$mul52,$mul51,$mul50,$iter20_idx,y,x;
		rnd = $m['Random'](113);
		under_curve = 0;
		$iter20_iter = $p['xrange'](Num_samples);
		$iter20_nextval=$p['__iter_prepare']($iter20_iter,false);
		while (typeof($p['__wrapped_next']($iter20_nextval)['$nextval']) != 'undefined') {
			count = $iter20_nextval['$nextval'];
			x = rnd['nextDouble']();
			y = rnd['nextDouble']();
			if ($p['bool'](($p['cmp']($p['__op_add']($add39=(typeof ($mul47=x)==typeof ($mul48=x) && typeof $mul47=='number'?
				$mul47*$mul48:
				$p['op_mul']($mul47,$mul48)),$add40=(typeof ($mul49=y)==typeof ($mul50=y) && typeof $mul49=='number'?
				$mul49*$mul50:
				$p['op_mul']($mul49,$mul50))), 1.0) < 1))) {
				under_curve = $p['__op_add']($add41=under_curve,$add42=1);
			}
		}
		return (typeof ($mul51=(typeof ($div17=$p['float'](under_curve))==typeof ($div18=Num_samples) && typeof $div17=='number' && $div18 !== 0?
			$div17/$div18:
			$p['op_div']($div17,$div18)))==typeof ($mul52=4.0) && typeof $mul51=='number'?
			$mul51*$mul52:
			$p['op_mul']($mul51,$mul52));
	};
	$m['MonteCarlo_integrate']['__name__'] = 'MonteCarlo_integrate';

	$m['MonteCarlo_integrate']['__bind_type__'] = 0;
	$m['MonteCarlo_integrate']['__args__'] = [null,null,['Num_samples']];
	$m['MonteCarlo'] = function(args) {
		var n;
		n = $p['float_int'](args['__getitem__'](0));
		$m['MonteCarlo_integrate'](n);
		return $p['sprintf']('MonteCarlo(%d)', n);
	};
	$m['MonteCarlo']['__name__'] = 'MonteCarlo';

	$m['MonteCarlo']['__bind_type__'] = 0;
	$m['MonteCarlo']['__args__'] = [null,null,['args']];
	$m['LU_factor'] = function(A, pivot) {
		var $augexpr1,$iter23_type,ab,$sub26,$iter22_array,$sub25,$iter25_array,$sub28,$sub29,M,$mul56,ii,$mul53,$iter23_idx,$iter22_type,$sub27,$add47,$div19,$add49,$add48,$iter23_iter,$add46,$add45,$add44,$add43,$iter24_type,$mul55,$iter24_iter,$iter24_nextval,$iter22_iter,$sub30,$iter21_idx,$augexpr2,$iter25_idx,jp,$iter21_nextval,$iter22_nextval,$augsub2,$augsub1,N,$iter21_iter,$iter23_array,jj,recp,$iter25_type,$add50,$div20,$iter23_nextval,$iter24_idx,minMN,k,j,$iter25_nextval,$iter22_idx,$iter25_iter,$mul54,$iter21_type,i,$iter24_array,t,$iter21_array;
		var $tupleassign6 = $p['__ass_unpack']($p['tuple']([$p['getattr'](A, 'height'), $p['getattr'](A, 'width')]), 2, null);
		M = $tupleassign6[0];
		N = $tupleassign6[1];
		minMN = $p['min'](M, N);
		$iter21_iter = $p['xrange'](minMN);
		$iter21_nextval=$p['__iter_prepare']($iter21_iter,false);
		while (typeof($p['__wrapped_next']($iter21_nextval)['$nextval']) != 'undefined') {
			j = $iter21_nextval['$nextval'];
			jp = j;
			t = $p['abs'](A['__getitem__'](j)['__getitem__'](j));
			$iter22_iter = $p['xrange']($p['__op_add']($add43=j,$add44=1), M);
			...
			while (typeof($p['__wrapped_next']($iter22_nextval)['$nextval']) != 'undefined') {
				...
				ab = $p['abs'](A['__getitem__'](i)['__getitem__'](j));
				if ($p['bool'](($p['cmp'](ab, t) == 1))) {
					jp = i;
					t = ab;
				}
			}
			pivot['__setitem__'](j, jp);
			if ($p['bool']($p['op_eq'](A['__getitem__'](jp)['__getitem__'](j), 0))) {
				throw ($p['Exception']('factorization failed because of zero pivot'));
			}
			if ($p['bool'](!$p['op_eq'](jp, j))) {
				var $tupleassign7 = $p['__ass_unpack']($p['tuple']([A['__getitem__'](jp), A['__getitem__'](j)]), 2, null);
				A['__setitem__'](j, $tupleassign7[0]);
				A['__setitem__'](jp, $tupleassign7[1]);
			}
			if ($p['bool'](($p['cmp'](j, $p['__op_sub']($sub25=M,$sub26=1)) == -1))) {
				recp = (typeof ($div19=1.0)==typeof ($div20=A['__getitem__'](j)['__getitem__'](j)) && typeof $div19=='number' && $div20 !== 0?
					$div19/$div20:
					$p['op_div']($div19,$div20));
				$iter23_iter = $p['xrange']($p['__op_add']($add45=j,$add46=1), M);
				$iter23_nextval=$p['__iter_prepare']($iter23_iter,false);
				while (typeof($p['__wrapped_next']($iter23_nextval)['$nextval']) != 'undefined') {
					k = $iter23_nextval['$nextval'];
					var $augsub1 = j;
					var $augexpr1 = A['__getitem__'](k);
					$augexpr1['__setitem__']($augsub1, (typeof ($mul53=$augexpr1['__getitem__']($augsub1))==typeof ($mul54=recp) && typeof $mul53=='number'?
						$mul53*$mul54:
						$p['op_mul']($mul53,$mul54)));
				}
			}
			if ($p['bool'](($p['cmp'](j, $p['__op_sub']($sub27=minMN,$sub28=1)) == -1))) {
				$iter24_iter = $p['xrange']($p['__op_add']($add47=j,$add48=1), M);
				$iter24_nextval=$p['__iter_prepare']($iter24_iter,false);
				while (typeof($p['__wrapped_next']($iter24_nextval)['$nextval']) != 'undefined') {
					ii = $iter24_nextval['$nextval'];
					$iter25_iter = $p['xrange']($p['__op_add']($add49=j,$add50=1), N);
					$iter25_nextval=$p['__iter_prepare']($iter25_iter,false);
					while (typeof($p['__wrapped_next']($iter25_nextval)['$nextval']) != 'undefined') {
						jj = $iter25_nextval['$nextval'];
						var $augsub2 = jj;
						var $augexpr2 = A['__getitem__'](ii);
						$augexpr2['__setitem__']($augsub2, $p['__op_sub']($sub29=$augexpr2['__getitem__']($augsub2),$sub30=(typeof ($mul55=A['__getitem__'](ii)['__getitem__'](j))==typeof ($mul56=A['__getitem__'](j)['__getitem__'](jj)) && typeof $mul55=='number'?
							$mul55*$mul56:
	...
	$m['LU_factor']['__name__'] = 'LU_factor';

	$m['LU_factor']['__bind_type__'] = 0;
	$m['LU_factor']['__args__'] = [null,null,['A'],['pivot']];
	$m['LU'] = function(args) {
		var A,$mul58,$iter26_iter,$iter26_idx,$iter26_nextval,i,$mul57,rnd,$iter26_type,N,lu,$iter26_array,pivot,cycles;
		var $tupleassign8 = $p['__ass_unpack']($p['map']($p['float_int'], args), 2, null);
		N = $tupleassign8[0];
		cycles = $tupleassign8[1];
		rnd = $m['Random'](7);
		A = rnd['RandomMatrix']($m['ArrayList'](N, N));
		lu = $m['ArrayList'](N, N);
		pivot = (typeof ($mul57=$m['array']('i', $p['list']([0])))==typeof ($mul58=N) && typeof $mul57=='number'?
			$mul57*$mul58:
			$p['op_mul']($mul57,$mul58));
		$iter26_iter = $p['xrange'](cycles);
		$iter26_nextval=$p['__iter_prepare']($iter26_iter,false);
		while (typeof($p['__wrapped_next']($iter26_nextval)['$nextval']) != 'undefined') {
			i = $iter26_nextval['$nextval'];
			lu['copy_data_from'](A);
			$m['LU_factor'](lu, pivot);
		}
		return $p['sprintf']('LU(%d, %d)', $p['tuple']([N, cycles]));
	};
	$m['LU']['__name__'] = 'LU';

	$m['LU']['__bind_type__'] = 0;
	$m['LU']['__args__'] = [null,null,['args']];
	$m['int_log2'] = function(n) {
		var $add51,$mul59,log,k,$mul60,$add52;
		k = 1;
		log = 0;
		while ($p['bool'](($p['cmp'](k, n) == -1))) {
			k = (typeof ($mul59=k)==typeof ($mul60=2) && typeof $mul59=='number'?
				$mul59*$mul60:
				$p['op_mul']($mul59,$mul60));
			log = $p['__op_add']($add51=log,$add52=1);
		}
		if ($p['bool'](!$p['op_eq'](n, (1)<<(log)))) {
			throw ($p['Exception']($p['sprintf']('FFT: Data length is not a power of 2: %s', n)));
		}
		return log;
	};
	$m['int_log2']['__name__'] = 'int_log2';

	$m['int_log2']['__bind_type__'] = 0;
	$m['int_log2']['__args__'] = [null,null,['n']];
	$m['FFT_num_flops'] = function(N) {
		var $mul62,$sub31,$sub32,$add54,$add55,$mul64,$mul63,$mul65,$add56,$mul61,$add53,$mul66;
		return $p['__op_add']($add55=(typeof ($mul63=$p['__op_sub']($sub31=(typeof ($mul61=5.0)==typeof ($mul62=N) && typeof $mul61=='number'?
			$mul61*$mul62:
			$p['op_mul']($mul61,$mul62)),$sub32=2))==typeof ($mul64=$m['int_log2'](N)) && typeof $mul63=='number'?
			$mul63*$mul64:
			$p['op_mul']($mul63,$mul64)),$add56=(typeof ($mul65=2)==typeof ($mul66=$p['__op_add']($add53=N,$add54=1)) && typeof $mul65=='number'?
			$mul65*$mul66:
			$p['op_mul']($mul65,$mul66)));
	};
	$m['FFT_num_flops']['__name__'] = 'FFT_num_flops';

	$m['FFT_num_flops']['__bind_type__'] = 0;
	$m['FFT_num_flops']['__args__'] = [null,null,['N']];
	$m['FFT_transform_internal'] = function(N, data, direction) {
		var $add89,$add88,$add83,$add82,$add81,$add80,$add87,$add86,$add85,$add84,$iter27_idx,$iter29_array,$iter27_iter,$mul80,$mul81,$mul82,$mul83,$mul84,$mul85,$mul86,$mul87,$sub39,$sub38,$augsub4,$augsub3,$mul72,$iter28_iter,bit,$add76,$add77,$add74,$add75,$add72,$add73,$add70,$iter28_array,$iter28_nextval,$add71,$add78,$add79,$augexpr3,t,$mul99,$mul98,$mul97,$mul96,$mul95,$mul94,$mul93,$mul92,$mul91,$mul90,logn,$add65,$iter27_nextval,$add67,$add66,$add61,$add60,$add63,$add62,$add69,$add68,theta,$iter27_type,$augexpr4,$mul104,$mul105,$mul106,$iter29_nextval,$mul100,$mul101,$mul102,$mul103,$iter29_idx,$div21,$div23,$div22,$div25,$div24,$add57,$add58,$add59,wd_imag,$iter29_type,s,w_real,tmp_real,$mul67,$mul68,$mul69,$div26,tmp_imag,z1_imag,$mul71,$mul70,$mul73,b,$mul75,$mul74,$mul77,$mul76,$mul79,$mul78,$iter29_iter,j,n,w_imag,wd_real,$iter28_idx,$sub33,$sub48,dual,$sub41,$sub40,$sub35,$sub42,$sub43,$sub44,$sub45,$sub46,$sub34,$sub37,$iter27_array,s2,$sub36,$mul88,$mul89,$sub47,$add64,$add94,$add90,$add91,$add92,$add93,$iter28_type,a,i,z1_real;
		n = (typeof ($div21=N)==typeof ($div22=2) && typeof $div21=='number' && $div22 !== 0?
			$div21/$div22:
		...
		(typeof FFT_bitreverse == "undefined"?$m['FFT_bitreverse']:FFT_bitreverse)(N, data);
		bit = 0;
		while ($p['bool'](($p['cmp'](bit, logn) == -1))) {
			w_real = 1.0;
			w_imag = 0.0;
			theta = (typeof ($div23=(typeof ($mul69=(typeof ($mul67=2.0)==typeof ($mul68=direction) && typeof $mul67=='number'?
				$mul67*$mul68:
				$p['op_mul']($mul67,$mul68)))==typeof ($mul70=$p['getattr']($m['math'], 'pi')) && typeof $mul69=='number'?
				$mul69*$mul70:
				$p['op_mul']($mul69,$mul70)))==typeof ($div24=(typeof ($mul71=2.0)==typeof ($mul72=$p['float'](dual)) && typeof $mul71=='number'?
				$mul71*$mul72:
				$p['op_mul']($mul71,$mul72))) && typeof $div23=='number' && $div24 !== 0?
				$div23/$div24:
				$p['op_div']($div23,$div24));
			s = $m['math']['sin'](theta);
			t = $m['math']['sin']((typeof ($div25=theta)==typeof ($div26=2.0) && typeof $div25=='number' && $div26 !== 0?
				$div25/$div26:
				$p['op_div']($div25,$div26)));
			s2 = (typeof ($mul75=(typeof ($mul73=2.0)==typeof ($mul74=t) && typeof $mul73=='number'?
				$mul73*$mul74:
				$p['op_mul']($mul73,$mul74)))==typeof ($mul76=t) && typeof $mul75=='number'?
				$mul75*$mul76:
				$p['op_mul']($mul75,$mul76));
			$iter27_iter = $p['range'](0, n, (typeof ($mul77=2)==typeof ($mul78=dual) && typeof $mul77=='number'?
				$mul77*$mul78:
				$p['op_mul']($mul77,$mul78)));
			$iter27_nextval=$p['__iter_prepare']($iter27_iter,false);
			while (typeof($p['__wrapped_next']($iter27_nextval)['$nextval']) != 'undefined') {
				b = $iter27_nextval['$nextval'];
				i = (typeof ($mul79=2)==typeof ($mul80=b) && typeof $mul79=='number'?
					$mul79*$mul80:
					$p['op_mul']($mul79,$mul80));
				j = (typeof ($mul81=2)==typeof ($mul82=$p['__op_add']($add57=b,$add58=dual)) && typeof $mul81=='number'?
					$mul81*$mul82:
					$p['op_mul']($mul81,$mul82));
				wd_real = data['__getitem__'](j);
				wd_imag = data['__getitem__']($p['__op_add']($add59=j,$add60=1));
				data['__setitem__'](j, $p['__op_sub']($sub33=data['__getitem__'](i),$sub34=wd_real));
				data['__setitem__']($p['__op_add']($add63=j,$add64=1), $p['__op_sub']($sub35=data['__getitem__']($p['__op_add']($add61=i,$add62=1)),$sub36=wd_imag));
				data['__setitem__'](i, $p['__op_add']($add65=data['__getitem__'](i),$add66=wd_real));
				var $augsub3 = $p['__op_add']($add67=i,$add68=1);
				var $augexpr3 = data;
				$augexpr3['__setitem__']($augsub3, $p['__op_add']($add69=$augexpr3['__getitem__']($augsub3),$add70=wd_imag));
			}
			$iter28_iter = $p['xrange'](1, dual);
			$iter28_nextval=$p['__iter_prepare']($iter28_iter,false);
			while (typeof($p['__wrapped_next']($iter28_nextval)['$nextval']) != 'undefined') {
				a = $iter28_nextval['$nextval'];
				tmp_real = $p['__op_sub']($sub39=$p['__op_sub']($sub37=w_real,$sub38=(typeof ($mul83=s)==typeof ($mul84=w_imag) && typeof $mul83=='number'?
					$mul83*$mul84:
					$p['op_mul']($mul83,$mul84))),$sub40=(typeof ($mul85=s2)==typeof ($mul86=w_real) && typeof $mul85=='number'?
					$mul85*$mul86:
					$p['op_mul']($mul85,$mul86)));
				tmp_imag = $p['__op_sub']($sub41=$p['__op_add']($add71=w_imag,$add72=(typeof ($mul87=s)==typeof ($mul88=w_real) && typeof $mul87=='number'?
					$mul87*$mul88:
					$p['op_mul']($mul87,$mul88))),$sub42=(typeof ($mul89=s2)==typeof ($mul90=w_imag) && typeof $mul89=='number'?
					$mul89*$mul90:
					$p['op_mul']($mul89,$mul90)));
				w_real = tmp_real;
				w_imag = tmp_imag;
				$iter29_iter = $p['range'](0, n, (typeof ($mul91=2)==typeof ($mul92=dual) && typeof $mul91=='number'?
					$mul91*$mul92:
					$p['op_mul']($mul91,$mul92)));
				$iter29_nextval=$p['__iter_prepare']($iter29_iter,false);
				while (typeof($p['__wrapped_next']($iter29_nextval)['$nextval']) != 'undefined') {
					b = $iter29_nextval['$nextval'];
					i = (typeof ($mul93=2)==typeof ($mul94=$p['__op_add']($add73=b,$add74=a)) && typeof $mul93=='number'?
						$mul93*$mul94:
						$p['op_mul']($mul93,$mul94));
					j = (typeof ($mul95=2)==typeof ($mul96=$p['__op_add']($add77=$p['__op_add']($add75=b,$add76=a),$add78=dual)) && typeof $mul95=='number'?
						$mul95*$mul96:
						$p['op_mul']($mul95,$mul96));
					z1_real = data['__getitem__'](j);
					z1_imag = data['__getitem__']($p['__op_add']($add79=j,$add80=1));
					wd_real = $p['__op_sub']($sub43=(typeof ($mul97=w_real)==typeof ($mul98=z1_real) && typeof $mul97=='number'?
						$mul97*$mul98:
						$p['op_mul']($mul97,$mul98)),$sub44=(typeof ($mul99=w_imag)==typeof ($mul100=z1_imag) && typeof $mul99=='number'?
						$mul99*$mul100:
						$p['op_mul']($mul99,$mul100)));
					wd_imag = $p['__op_add']($add81=(typeof ($mul101=w_real)==typeof ($mul102=z1_imag) && typeof $mul101=='number'?
						$mul101*$mul102:
						$p['op_mul']($mul101,$mul102)),$add82=(typeof ($mul103=w_imag)==typeof ($mul104=z1_real) && typeof $mul103=='number'?
						$mul103*$mul104:
						$p['op_mul']($mul103,$mul104)));
					data['__setitem__'](j, $p['__op_sub']($sub45=data['__getitem__'](i),$sub46=wd_real));
					data['__setitem__']($p['__op_add']($add85=j,$add86=1), $p['__op_sub']($sub47=data['__getitem__']($p['__op_add']($add83=i,$add84=1)),$sub48=wd_imag));
					data['__setitem__'](i, $p['__op_add']($add87=data['__getitem__'](i),$add88=wd_real));
					var $augsub4 = $p['__op_add']($add89=i,$add90=1);
					var $augexpr4 = data;
					$augexpr4['__setitem__']($augsub4, $p['__op_add']($add91=$augexpr4['__getitem__']($augsub4),$add92=wd_imag));
				}
			}
			bit = $p['__op_add']($add93=bit,$add94=1);
			dual = (typeof ($mul105=dual)==typeof ($mul106=2) && typeof $mul105=='number'?
				$mul105*$mul106:
				$p['op_mul']($mul105,$mul106));
		}
		return null;
	};
	$m['FFT_transform_internal']['__name__'] = 'FFT_transform_internal';

	$m['FFT_transform_internal']['__bind_type__'] = 0;
	$m['FFT_transform_internal']['__args__'] = [null,null,['N'],['data'],['direction']];
	$m['FFT_bitreverse'] = function(N, data) {
		var nm1,$add103,ii,$sub49,$iter30_array,$add104,$sub52,$iter30_nextval,$add101,$add100,$iter30_type,$add102,$add98,$add99,$sub51,$sub50,$add95,$add96,$add97,$iter30_idx,jj,tmp_imag,$div27,$iter30_iter,$div28,k,j,n,i,tmp_real;
		n = (typeof ($div27=N)==typeof ($div28=2) && typeof $div27=='number' && $div28 !== 0?
			$div27/$div28:
			$p['op_div']($div27,$div28));
		nm1 = $p['__op_sub']($sub49=n,$sub50=1);
		j = 0;
		$iter30_iter = $p['range'](nm1);
		$iter30_nextval=$p['__iter_prepare']($iter30_iter,false);
		while (typeof($p['__wrapped_next']($iter30_nextval)['$nextval']) != 'undefined') {
			...
			jj = (j)<<(1);
			k = (n)>>(1);
			if ($p['bool'](($p['cmp'](i, j) == -1))) {
				tmp_real = data['__getitem__'](ii);
				tmp_imag = data['__getitem__']($p['__op_add']($add95=ii,$add96=1));
				data['__setitem__'](ii, data['__getitem__'](jj));
				data['__setitem__']($p['__op_add']($add99=ii,$add100=1), data['__getitem__']($p['__op_add']($add97=jj,$add98=1)));
				data['__setitem__'](jj, tmp_real);
				data['__setitem__']($p['__op_add']($add101=jj,$add102=1), tmp_imag);
			}
			while ($p['bool'](($p['cmp'](k, j) < 1))) {
				j = $p['__op_sub']($sub51=j,$sub52=k);
				k >>= 1;
			}
			j = $p['__op_add']($add103=j,$add104=k);
		}
		return null;
	};
	$m['FFT_bitreverse']['__name__'] = 'FFT_bitreverse';

	$m['FFT_bitreverse']['__bind_type__'] = 0;
	$m['FFT_bitreverse']['__args__'] = [null,null,['N'],['data']];
	$m['FFT_transform'] = function(N, data) {

		$m['FFT_transform_internal'](N, data, (typeof ($usub1=1)=='number'?
			-$usub1:
			$p['op_usub']($usub1)));
		return null;
	};
	$m['FFT_transform']['__name__'] = 'FFT_transform';

	$m['FFT_transform']['__bind_type__'] = 0;
	$m['FFT_transform']['__args__'] = [null,null,['N'],['data']];
	$m['FFT_inverse'] = function(N, data) {
		var $div32,$div30,$div31,$iter31_array,$div29,$mul108,$iter31_nextval,$iter31_idx,n,i,$mul107,$iter31_type,norm,$iter31_iter;
		n = (typeof ($div29=N)==typeof ($div30=2) && typeof $div29=='number' && $div30 !== 0?
			$div29/$div30:
			$p['op_div']($div29,$div30));
		norm = 0.0;
		$m['FFT_transform_internal'](N, data, (typeof ($uadd1=1)=='number'?
			$uadd1:
			$p['op_uadd']($uadd1)));
		norm = (typeof ($div31=1)==typeof ($div32=$p['float'](n)) && typeof $div31=='number' && $div32 !== 0?
			$div31/$div32:
			$p['op_div']($div31,$div32));
		$iter31_iter = $p['xrange'](N);
		$iter31_nextval=$p['__iter_prepare']($iter31_iter,false);
		while (typeof($p['__wrapped_next']($iter31_nextval)['$nextval']) != 'undefined') {
			i = $iter31_nextval['$nextval'];
			data['__setitem__'](i, (typeof ($mul107=data['__getitem__'](i))==typeof ($mul108=norm) && typeof $mul107=='number'?
				$mul107*$mul108:
				$p['op_mul']($mul107,$mul108)));
		}
		return null;
	};
	$m['FFT_inverse']['__name__'] = 'FFT_inverse';

	$m['FFT_inverse']['__bind_type__'] = 0;
	$m['FFT_inverse']['__args__'] = [null,null,['N'],['data']];
	$m['FFT'] = function(args) {
		var twoN,$iter32_idx,$iter32_nextval,i,N,$iter32_type,$iter32_iter,$iter32_array,$mul110,x,cycles,$mul109;
		var $tupleassign9 = $p['__ass_unpack']($p['map']($p['float_int'], args), 2, null);
		N = $tupleassign9[0];
		cycles = $tupleassign9[1];
		twoN = (typeof ($mul109=2)==typeof ($mul110=N) && typeof $mul109=='number'?
			$mul109*$mul110:
			$p['op_mul']($mul109,$mul110));
		x = $m['Random'](7)['RandomVector'](twoN);
		$iter32_iter = $p['xrange'](cycles);
		$iter32_nextval=$p['__iter_prepare']($iter32_iter,false);
		while (typeof($p['__wrapped_next']($iter32_nextval)['$nextval']) != 'undefined') {
			i = $iter32_nextval['$nextval'];
			$m['FFT_transform'](twoN, x);
			$m['FFT_inverse'](twoN, x);
		}
		return $p['sprintf']('FFT(%d, %d)', $p['tuple']([N, cycles]));
	};
	$m['FFT']['__name__'] = 'FFT';

	$m['FFT']['__bind_type__'] = 0;
	$m['FFT']['__args__'] = [null,null,['args']];
	$m['FFT']($p['list']([1024, 100]));
	return this;
}; /* end main */
