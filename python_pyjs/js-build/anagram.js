...
$pyjs['loaded_modules']['main'] = function (__mod_name__) {
	...
	$m['scrambler'] = function(word) {
		var $iter1_iter,$$char,$iter1_nextval,n,$iter2_iter,tail,$iter1_array,$iter2_type,$iter2_idx,nstrings,finals,$iter2_nextval,$iter1_type,i,uniqueList,$add2,$add3,$add1,$add6,$iter1_idx,$add4,$add5,$iter2_array,newfinal;
		if ($p['bool']($p['op_eq']($p['len'](word), 1))) {
			nstrings = $p['list']([word]);
			return nstrings;
		}
		else {
			uniqueList = $p['list'](word);
			finals = $p['list']([]);
			$$char = uniqueList['__getitem__'](0);
			tail = ''['join']($p['__getslice'](uniqueList, 1, null));
			nstrings = $p['list']($p['set']($m['scrambler'](tail)));
			$iter1_iter = nstrings;
			$iter1_nextval=$p['__iter_prepare']($iter1_iter,false);
			while (typeof($p['__wrapped_next']($iter1_nextval)['$nextval']) != 'undefined') {
				n = $iter1_nextval['$nextval'];
				$iter2_iter = $p['range'](0, $p['__op_add']($add1=$p['len'](n),$add2=1));
				$iter2_nextval=$p['__iter_prepare']($iter2_iter,false);
				while (typeof($p['__wrapped_next']($iter2_nextval)['$nextval']) != 'undefined') {
					i = $iter2_nextval['$nextval'];
					newfinal = $p['__op_add']($add5=$p['__op_add']($add3=$p['__getslice'](n, 0, i),$add4=$$char),$add6=$p['__getslice'](n, i, $p['len'](n)));
					finals['append'](newfinal);
				}
			}
			return $p['list']($p['set'](finals));
		}
		return null;
	};
	$m['scrambler']['__name__'] = 'scrambler';

	$m['scrambler']['__bind_type__'] = 0;
	$m['scrambler']['__args__'] = [null,null,['word']];
	$m['scrambles'] = $m['scrambler']('berrycakes');
	$p['printFunc']([$p['len']($m['scrambles'])], 1);
	return this;
}; /* end main */
