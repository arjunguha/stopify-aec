...
(defn dot [r i]
  (let [f2 (double 2.0)
    ...
    (loop [zr (double 0.0)
           zi (double 0.0)
           i (int (unchecked-inc-int iterations-remaining))]
      (let [zr2 (* zr zr)
            zi2 (* zi zi)]
        (if (and (not (zero? i))
...
(defn compute-row [#^doubles x-vals y]
  (let [y (double y)
        ...
        num-bytes-out (int (unchecked-divide-int (unchecked-add-int n 7) 8))
        #^bytes result (make-array num-bytes-out)
        zero (int 0)
        one (int 1)]
    (loop [i (int zero)
           b (int zero)
      ...
      (if (== i n)
        ...
        (let [new-bit (int (if (dot (aget x-vals i) y) one zero))
              new-b (int (unchecked-add-int (bit-shift-left b one) new-bit))]
          (if (== num-filled-bits 7)
            (let [byte-val (byte (ubyte new-b))]
              ...
              (recur (unchecked-inc-int i)
                     zero
                     zero
                     (unchecked-inc-int result-idx)))
            (recur (unchecked-inc-int i)
                   new-b
                   (unchecked-inc-int num-filled-bits)
                   result-idx)))))))
...
(defn do-mandelbrot [size]
  (let [rows (compute-rows size)]
    (cl-format true "P4")
    (cl-format true "~d ~d" size size)
    (doseq [r rows]
      ;; iterate through all the rows to increase benchmark time
      (+ 1 2))
