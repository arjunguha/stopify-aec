...
(defprotocol IBody
  (p! [this ^double x ^double y ^double z] "set position")
  (p-dt! [this ^double dt] "update position")
  (v-dt! [this ^double dt other] "update velocity")
  (v! [this ^double vx ^double vy ^double vz] "set velocity")
  (v+! [this ^double vx ^double vy ^double vz] "add to velocity")
  (get-energy [this nbodies] "energy relative to nbodies"))
...
(deftype Body [^String name
  ...
  Object
  (toString [this]
    (let [fmt " ~s mass ~,3f~% p: [~,3f ~,3f ~,3f]~% v: [~,3f ~,3f ~,3f]~%"]
      (cl-format nil fmt name mass x y z vx vy vz))) )
...
(defn energy
  ...
  ([bodies]              ; loop
    (if-not bodies 0.0
      (+ (get-energy (first bodies) (next bodies))
        (energy (next bodies))))))
...
(defn advance [dt]
  ...
  (let [bodies bodies
        ...
        dt (double dt) ]
    (dotimes [i len]
      (let [body (aget bodies i) ]
        ; update velocity
        (loop [j (unchecked-inc-int i)]
          (when (< j len)
            (let [nbody (aget bodies j)]
              (v-dt! body dt nbody)
