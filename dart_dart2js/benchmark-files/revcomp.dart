...
class Revcomp extends BenchmarkBase {
  const Revcomp() : super('Revcomp');
  void run() {
    ...
    inputArray.map((line) => line + "\n").forEach((List<int> dataList) {
      // Loop over all the contents of the buffer so far
      for (int data in dataList.runes) {

        // Check if this is a comment line (and that we aren't already on a
        // comment line)
